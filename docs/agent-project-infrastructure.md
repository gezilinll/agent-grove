...
| OpenAI Harness Engineering | <https://openai.com/index/harness-engineering/> | 2026-02-11 | agent-first 仓库、可观测性、反馈闭环 |
| OpenAI Codex App Server | <https://openai.com/index/unlocking-the-codex-harness/> | 2026-02-04 | agent loop、thread/turn/item、approval protocol |
| OpenAI Symphony | <https://openai.com/index/open-source-codex-orchestration-symphony/> | 2026-04-27 | orchestration spec、`WORKFLOW.md`、bounded concurrency |
...
| Anthropic Effective Harnesses | Anthropic Engineering | 2025-11-26 | initializer/coding agent、feature list、progress handoff |
| Anthropic Demystifying Evals | Anthropic Engineering | 2026-01-09 | agent harness 与 evaluation harness |
| Anthropic Harness Design | Anthropic Engineering | 2026-03-24 | planner/generator/evaluator、sprint contract、QA loop |
| Anthropic Context Engineering | Anthropic Engineering | 2025-09-29 | context 有限性、上下文策展 |
...
这个矩阵给出一个初步判断：本次样本显示，业界在 **Execution Harness、Tool Boundary、Context Architecture、Evaluation Harness** 上已经有较强收敛，但 **Project Instructions** 和 **Task / Workflow Spec** 还没有统一形态。前者正在向 `AGENTS.md` / `.agents/` 靠拢，后者仍然在 Markdown、YAML、graph、DSL、dataset schema 之间分化。
...
Execution Harness 是让 Agent 能在可重复环境里执行、观察、修正的系统。OpenAI 的 harness engineering 文章把问题上升为工程角色变化：人类更多设计环境、指定意图、构建反馈回路；仓库知识、可观测性、测试入口都成为 Agent 能力的一部分。Anthropic 的长任务 harness 案例进一步说明，harness 还包括任务分解、结构化交接、端到端验证，以及 generator / evaluator 分工。
...
OpenAI Codex App Server 展示了另一种官方视角：harness 不只在 CLI 内部运行，也可以被抽象成稳定协议，暴露给 IDE、桌面端、Web runtime 和第三方集成。

```text
thread: 持久会话，可创建、恢复、fork、archive
turn: 一次用户输入触发的一段 Agent 工作
item: tool execution、approval request、diff、message 等事件单元
```

这个设计的关键点是：长任务不是简单 request / response。一次 turn 会产生多组事件，工具调用可以请求 approval 并暂停当前 turn，客户端再以 allow / deny 恢复执行。
...
Anthropic 的 long-running harness 则提醒我们：扩大 context window 或做 compaction 不等于解决长任务。它把长任务拆成 initializer agent 和 coding agent：前者创建 `init.sh`、feature list、progress file 和初始 git commit；后者每次先读进度、git log 和 feature list，只实现一个特性，最后留下可交接状态。2026 年的 follow-up 又把这个思路推进为 planner / generator / evaluator，要求每个 sprint 先形成 contract，再由 evaluator 用 Playwright MCP 点击运行中的应用、检查 API 和数据库状态。
...
Evaluation Harness 负责“怎么判”，Execution Harness 负责“怎么跑”。Anthropic 明确区分 agent harness 和 evaluation harness：前者让模型能作为 Agent 行动，后者端到端运行任务、记录步骤、评分并汇总结果。OpenAI Agent Evals 的建议也类似：还在调试行为时先从 trace grading 开始，知道“什么是好”之后再沉淀为 dataset 和 eval run。
...
本次样本里的共性正在形成：eval 必须同时看结果和过程。trace、tool call、dataset、score、rubric 是高频组成件。
...
MCP 的 tool / resource / prompt 分工提供了一个底层视角：tool 是 model-controlled 的行动能力，resource 是 application-driven 的上下文来源。把只读数据建模成 tool，会放大权限面；把有副作用的 tool 当普通函数，会低估风险。MCP 规范还强调，涉及信任和安全时，应用应该让用户看见暴露给模型的工具，并对高风险调用保留人工确认能力。
...
Context Architecture 回答“Agent 应该知道什么，以及这些信息何时进入上下文”。Anthropic 将 context engineering 视为 prompt engineering 之后的自然延伸：问题不只是写什么指令，而是每一次推理时如何策展有限 token。OpenAI Harness Engineering 也给出类似判断：给 Agent 一张地图，而不是一份 1000 页手册。
...
同时，harness 需要被持续复核。Anthropic 2026 年的 harness 文章有一个重要提醒：模型能力变化后，原来 load-bearing 的脚手架可能会变成成本和延迟负担；但模型越强，也会打开新的 harness 组合空间。Agent Grove 的实践应记录每个组件解决的失败模式，并定期删掉不再必要的复杂度。
